...
export default function HeadLayout() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
            {/* Header */}
            <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200 px-3 sm:px-4 py-2.5 shadow-sm">
                <div className="flex items-center gap-2 sm:gap-3 max-w-7xl mx-auto">
                    {/* Left: Search Bar (1/3 width on desktop) */}
                    <div className="relative flex-1 max-w-xs">
                        ...
                        <Input
                            ...
                            className="pl-9 h-9 rounded-xl border-slate-200 bg-slate-50/50 focus:bg-white text-sm"
                        />
                    </div>
                        ...
                        {NAV_ITEMS.map((item) => {
                            ...
                            return (
                                <NavLink
                                    key={item.to}
                                    to={item.to}
                                    className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-semibold transition-all duration-300 ${isActive
                    ...
                    <Button
                        variant="ghost"
                        size="sm"
                        className="text-red-600 hover:bg-red-50 hover:text-red-700 gap-2 pl-2 pr-3 h-9 shrink-0"
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 pt-1.5 pb-4 px-4 shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.1)] z-40 backdrop-blur-xl bg-white/90">
                <div className="flex items-center justify-between">
                        ...
                        {NAV_ITEMS.map((item) => {
                            ...
                            return (
                                <NavLink
                                    key={item.to}
                                    to={item.to}
                                    className={`flex flex-col items-center gap-0.5 min-w-[56px] group ${isActive ? "text-primary" : "text-slate-400 hover:text-slate-600"
                                        }`}
                                >
                                    <div className={`p-1.5 rounded-xl transition-all duration-300 ${isActive
                                        ...
                                        <Icon className={`h-5 w-5 ${isActive ? "fill-primary/20 stroke-[2.5px]" : ""}`} />
                                    </div>
                                    <span className={`text-[9px] font-bold transition-all`}>{item.label}</span>
                                </NavLink>
                            );
                        })}
                    </div>
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className="text-slate-500 hover:text-primary hover:bg-primary/5 rounded-xl h-8 w-8 shrink-0 ml-2"
